...
void rustc_new(WrenVM* vm) {
	...
	FILE* const fd = fopen(LSP_CONFIG, "w");

	if (!fd) {
		LOG_ERROR("Failed to create RustC LSP configuration file");
		return;
	}

	validate_gitignore(LSP_CONFIG);
	validate_gitignore("Cargo.lock");
	validate_gitignore("target"); // this is needed, as we have no way of telling rust-analyzer what our actual target directory is

	// create the virtual manifest file which will tell rust-analyzer what to do

	fprintf(fd, "# this file is automatically generated by Bob the Builder\n");
	fprintf(fd, "# you should not attempt to run 'cargo x' directly\n");
	fprintf(fd, "# this is just a stopgap until rust-project.json support is figured out 😄\n");
	fprintf(fd, "# see https://github.com/inobulles/bob/pull/49 for more info\n\n");
	fprintf(fd, "[workspace]\n");
	fprintf(fd, "members = [\"bin/*_Cargo.toml.d\"]\n");
