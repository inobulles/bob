...
static int link_step(size_t data_count, void** data) {
	...
	// Re-link if any flags, linked static libraries, or sources have changed too.

	bool do_link;

	if (frugal_link(&do_link, bss->log_prefix, bss->state->flags, src_count, srcs, out) < 0) {
		goto link;
	}
...
static int instantiate(flamingo_val_t* inst, flamingo_arg_list_t* args) {
	...
	for (size_t i = 0; i < flags->vec.count; i++) {
		...
		case FLAMINGO_VAL_KIND_INST:
			...
			LOG_FATAL(
				LINKER ": Expected %zu-th vector element to be a string or 'PkgConfigCookie', not '%.*s' instance.",
				i,
			...
			return -1;
		default:
			LOG_FATAL(LINKER ": Expected %zu-th vector element to be a string or 'PkgConfigCookie()'", i);
			return -1;
		}
	}
